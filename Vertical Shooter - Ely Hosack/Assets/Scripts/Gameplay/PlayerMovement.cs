...
public class PlayerMovement : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if(Input.anyKey && !GameManager.singleton.playing && !GameManager.singleton.gameOver)
        {
            GameManager.singleton.Playing();
            //lerp to initial position
            
        }

        if(GameManager.singleton.playing)
        {
